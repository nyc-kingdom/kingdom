            ...
            >
                {
                    ...
                    <div className='powerButton'>
                        YOU ARE NOW WORKING IN HACKED MODE - USER MAY NOW CHECK-IN WHEREVER THEY CHOOSE
                    </div>
                }
                ...
                <button
                    ...
                    style={{position: 'absolute', right: 0, top: '35vh', maxWidth: '24vw'}}
                >
                    <img src={gem}/>
                    TURN OFF CHECK-IN VERIFICATION
                </button>
                {
                    this.props.location.status === 'FINDINGLOCATION' &&
                    <img
                        style={{display:'block', padding: '5vh'}}
                        src={knightsword}
                    />
                }
                <input
                ...
                </button>
                {
                    ...
                    this.props.markers.map(eachMarker => (
                        <div className='listicle'>
                            <Link
                                key={eachMarker.venue.id}
                                to={`/dashboard/selectedView/${eachMarker.venue.id}`}
                            >
                                <div className='queryRec'>
                                    {eachMarker.venue.name} {eachMarker.venue.rating}
                                </div>
                            </Link>
                            <div className='subHeader'>
                                {eachMarker.venue.categories[0].name}
                            </div>
                        </div>
                    ))
                }
